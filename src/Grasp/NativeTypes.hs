...
module Grasp.NativeTypes
  ...
  -- * Display
  , showGraspType
  -- * RuntimeCheck re-exports
  , RepTag(..), readRepTag, getInfoPtr
  ) where
...
import Grasp.RuntimeCheck (RepTag(..), readRepTag, getInfoPtr)
...
data GraspPromptTag = GraspPromptTag Any
...
data GraspType
  ...
  | GTLambda | GTPrim | GTLazy | GTMacro | GTChan | GTModule | GTRecur | GTPromptTag | GTTVar
  deriving (Eq, Show)
...
showGraspType GTTVar      = "tvar"

-- ─── Info pointer cache ───────────────────────────────────
-- NOINLINE prevents GHC from inlining literal addresses.
...
-- ─── Type discrimination ─────────────────────────────────
-- Built on readRepTag: uses info pointer for identity.

graspTypeOf :: Any -> GraspType
graspTypeOf v = let p = rtInfoPtr (readRepTag v) in
  ...
  else if p == tvarInfoPtr      then GTTVar
